...
def read_log(log):
    ...
    QMs['name'] = log.name.strip('.log') if log.name is not None else np.nan
    QMs['type'] = jtype
    QMs['G'] = log.G if log.G is not None else np.nan
    QMs['H298'] = log.H_DFT if log.H_DFT is not None else np.nan
    QMs['S298'] = log.S_DFT if log.S_DFT is not None else np.nan
    QMs['time'] = log.CPU if log.CPU is not None else np.nan
