...
def parser(ts_id, tar_file_path):
...
for ts_id in ts_ids:
    ...
    if os.path.exists(tar_file_path):
        tar_file_paths.append(tar_file_path)
        ts_id_save.append(ts_id)

out = Parallel(n_jobs=n_jobs, backend="multiprocessing", verbose=5)(delayed(parser)(ts_id, tar_file) for ts_id, tar_file in zip(ts_id_save, tar_file_paths))
